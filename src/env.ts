...
const envSchema = z
  ...
  .superRefine((values, ctx) => {
    const accessKeyId =
      values.AWS_ACCESS_KEY_ID?.trim() || values.AWS_ACCESS_KEY?.trim();

    if (!accessKeyId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["AWS_ACCESS_KEY_ID"],
        message: "AWS_ACCESS_KEY_ID (or legacy AWS_ACCESS_KEY) is required.",
      });
    }
  });
...
export const env = {
  ...parsedEnv,
  AWS_ACCESS_KEY_ID:
    parsedEnv.AWS_ACCESS_KEY_ID?.trim() || parsedEnv.AWS_ACCESS_KEY?.trim() || "",
};
